...
if __name__ == '__main__':
    ...
    for tmp_index in range(INDEX_START, INDEX_END + 1):
        ...
        # remove pts that are not in FOV
        for idx, config in enumerate(camera_configs):
            mask = np.ones((outputRes[1], outputRes[0]), dtype=np.bool)
            R = cams[idx].R
            yaw_plan = 90 + np.rad2deg(np.arctan2(np.linalg.inv(R)[1, 0], np.linalg.inv(R)[0, 0]))
            for i in range(outputRes[0]):
                for j in range(outputRes[1]):
                    theta = np.rad2deg(np.arctan2(-j + outputRes[1] / 2, i - outputRes[0] / 2))
                    if 90 < abs(theta - yaw_plan) < 270:
                        mask[j, i] = False

            # 这里resize成指定大小
            mask = cv2.resize(mask.astype(np.uint8),(64,32),interpolation=cv2.INTER_NEAREST).astype(np.bool)
            masks.append(mask)

        masks = np.stack(masks)
        np.save("../../data/mask_nuscene.npy", masks)
        break
