...
if __name__ == '__main__':
    ...
    vis_dir = os.path.join(NUSCENE_DATA, str(INDEX_START) + "_" + str(INDEX_END) + "_final_label")
    ...
    out = cv2.VideoWriter(video_path, codec, 2, OUTPUT_VIDEO_SIZE)
    ...
    for tmp_index in range(INDEX_START, INDEX_END + 1):
        ...
        # bev label loading
        label_dir = os.path.join(vis_dir, str(tmp_index)+ ".png")
        ...
        cv2.rectangle(bev_label, (int(bev_width/2) - M2, int(bev_height/2) - M1),
                      (int(bev_width/2) + M2, int(bev_height/2) + M1), (255, 0, 0), -1)
        ...
        index_range = np.unique(bev_label[:,:,0]).shape[0]
        bev_label_single = bev_label[:,:,0].copy()
        for tmp_idx in range(index_range):
            tmp_mask = (bev_label_single==tmp_idx * BEV_VIS_MULTIPLY)
            bev_label[tmp_mask,:] = COLOR_VIS_LIST[tmp_idx]
