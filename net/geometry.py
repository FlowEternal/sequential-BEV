...
from net.affine_grid import affine_grid_own
...
def warp_features(x, flow, mode='nearest', spatial_extent=None, export=False):
    ...
    if export:
        grid = affine_grid_own(transformation, x)
        warped_x = bilinear_sample_noloop(x, grid.float())
...
def cumulative_warp_features(x, flow, mode='nearest', spatial_extent=None, export=False):
    ...
    for t in reversed(range(sequence_length - 1)):
        out.append(warp_features(x[:, t], mat2pose_vec(cum_flow), mode=mode, spatial_extent=spatial_extent,export=export))
